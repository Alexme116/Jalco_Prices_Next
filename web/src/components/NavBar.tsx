...
export default function NavBar(
    { user, isDevice } :
    { user: UserType | null; isDevice: string; }
) {
    ...
    useEffect(() => {
        ...
        if (path.includes("/userManager")) {
            ...
            setActualWindow("/report")
        } else if (path.includes("/authenticated")) {
                    ...
                    <div className={`flex items-center gap-10 transition-all duration-500 overflow-hidden ${user == null ? "w-0" : user.rol == "admin" ? "w-[290px]" : "w-[92px]"}`}>
                        ...
                        {isDevice != "Mobile" &&
                            <button
                                ...
                                className="hover:cursor-pointer"
                            >
                                <IconDashboard color={actualWindow == "/authenticated" ? "#edbd63" : "#77797a"} w={"26px"} h={"26px"}/>
                            </button>
                        }
            ...
            {showSettings && user &&
                <>
                    <div className="z-30 absolute top-16 right-0 flex flex-col items-center gap-5 p-5 rounded-bl-lg bg-white">
                        {/* Settings Title */}
                        <h1 className="md:text-lg">{user?.rol == "admin" ? "Administrador" : "Empleado"}</h1>

                        {/* Settings Dashboard */}
                        {isDevice == "Mobile" &&
                            <button className="flex gap-2 items-center hover:cursor-pointer" onClick={() => handleChangeView("/authenticated")}>
                                <div>
                                    <IconDashboard color={actualWindow == "/authenticated" ? "#edbd63" : "#77797a"}  w={"20px"} h={"20px"}/>
                                </div>
                        ...
                        {/* Settings Chat */}
                        {isDevice == "Mobile" &&
                            <button className="flex gap-2 items-center hover:cursor-pointer" onClick={() => handleChangeView("/authenticated/chat")}>
                                <div>
                        ...
                        {/* Settings Report */}
                        {isDevice == "Mobile" && user?.rol == "admin" &&
                            <button className="flex gap-2 items-center hover:cursor-pointer" onClick={() => handleChangeView("/authenticated/report")}>
                                <div>
                        ...
                        {/* Setting Logout */}
                        <button className="w-full px-3 py-2 bg-black text-white rounded-lg shadow-r-0 hover:cursor-pointer" onClick={() => handleChangeView("/")}>
